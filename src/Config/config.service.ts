...
class ConfigService {
	...
	constructor() {
		...
		this.generateSchema =
			`${process.env[GENERATE_SCHEMA]}`.toLowerCase()[0] === "t";
		...
		this.emailVerifyTemplateId = process.env[
			EMAIL_VERIFY_TEMPLATE_ID
		] as string;
		this.emailForgotPasswordTemplateId = process.env[
			EMAIL_FORGOT_PASSWORD_TEMPLATE_ID
		] as string;
		this.emailNotificationTemplateId = process.env[
